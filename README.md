...
- этапы видны всем участникам проекта через `project_members`, а отдельный
  `task_members` сохраняет task-scoped доступ для чек-листов, истории и
  назначений;
...
| Работа с пунктами чек-листа | да | да | да, если есть доступ к этапу | нет |
...
- наличие `project_members` даёт видимость строки любого этапа проекта;
- `task_members` остаётся отдельным task-scoped доступом: его видят участники
  этапа и owner/admin проекта, а mutating-операции продолжают его проверять;
- assignee обязан быть одновременно участником проекта и этапа;
