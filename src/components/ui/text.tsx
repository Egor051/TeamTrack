...
export function ThemedText({ type = 'body', style, children, ...rest }: ThemedTextProps) { const { colors: theme } = useTheme(); return <Text accessibilityRole={type === 'title' || type.startsWith('h') ? 'header' : undefined} style={[styles[type], { color: type === 'small' || type === 'caption' ? theme.textSecondary : theme.text }, style]} {...rest}>{children}</Text>; }
