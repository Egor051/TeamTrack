...
export default function ProjectsScreen() {
  ...
  return <Screen padded={false} centerContent={false}><ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={load} tintColor={theme.primary} />} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <View style={styles.topbar}><View style={styles.brand}><ThemedText type="h1">Проекты</ThemedText><ThemedText type="small">{state.profile?.display_name || state.user?.email}</ThemedText></View><View style={styles.nav}><Link href={'/templates' as never} asChild><Button size="sm" variant="outline">Шаблоны</Button></Link><Link href={'/notifications' as never} asChild><Button size="sm" variant="outline" accessibilityLabel="Уведомления">Уведомления{unread ? ` · ${unread}` : ''}</Button></Link><Link href={'/profile' as never} asChild><Button size="sm" variant="ghost">Профиль</Button></Link></View></View>
    <View style={styles.intro}><View><ThemedText type="h2">Проекты</ThemedText><ThemedText type="small">Команды и задачи в одном месте.</ThemedText></View><Button onPress={() => router.push('/projects/new' as never)}>Создать проект</Button></View>
    ...
    {!archived ? <View style={styles.section}><ThemedText type="h2">Мои задачи</ThemedText>{myTasks.length ? <View style={styles.grid}>{myTasks.map((task) => <Card key={task.id} onPress={() => router.push(`/projects/${task.project_id}/tasks/${task.id}` as never)}><View style={styles.cardHeader}><ThemedText type="h3" style={styles.flex}>{task.title}</ThemedText><Badge tone="primary">Моя задача</Badge></View><ThemedText type="small">{task.project_name}</ThemedText></Card>)}</View> : <EmptyState title="Нет назначенных задач" description="Здесь появятся активные задачи, где вы исполнитель." />}</View> : null}
