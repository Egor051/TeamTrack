...
export default function ProjectsScreen() {
  ...
  const load = useCallback(async () => { const request = ++requestRef.current; setLoading(true); setError(''); try { const [next, mine] = await Promise.all([listProjects(archived), archived ? Promise.resolve([] as MyTask[]) : listMyTasks(userId!)]); if (request !== requestRef.current) return; setProjects(next); setMyTasks(mine); } catch (e) { if (request === requestRef.current) setError(userMessage(e, 'Не удалось загрузить проекты.')); } finally { if (request === requestRef.current) setLoading(false); } }, [archived, userId]);
  ...
  return <Screen padded={false} centerContent={false}><ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={load} tintColor={colors.primary} />} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    {!archived ? <View style={styles.mine}><ThemedText type="h2">Мои задачи</ThemedText>{myTasks.length ? <View style={styles.grid}>{myTasks.map((task) => <Card key={task.id} onPress={() => router.push(`/projects/${task.project_id}/tasks/${task.id}` as never)}><View style={styles.cardHeader}><ThemedText type="h3" style={styles.flex}>{task.title}</ThemedText><Badge tone="primary">Моя задача</Badge></View><ThemedText type="small">{task.project_name}</ThemedText></Card>)}</View> : <EmptyState title="Назначенных задач нет" description="Здесь появятся активные задачи, где вы исполнитель." />}</View> : null}
